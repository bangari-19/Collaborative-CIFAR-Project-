...
coeff_draw_from_cov <- function(amplitude, mat, cov_mat, mask) {
  print(amplitude)
  size <- dim(mat)[1]  
  sprintf('size of individual\'s matrix: %d', size)
  print(cov_mat)
  random_mat <- amplitude * matrix(rnorm(size**2), nrow = size)
  print(matrix(rnorm(size**2), nrow = size))
  print(random_mat)
  sample_step <- cov_mat %*% random_mat  # generate noise
  print(sample_step)
  cat("sample step type", typeof(sample_step))
...
# clip_outliers
clip_outliers <- function(timeseries, sigma, measure_amp, debug = F) {
  ts <- matrix(timeseries, nrow=nrow(timeseries), ncol=ncol(timeseries))
  for (i in seq_len(ncol(timeseries)) - 1) {
    min <- mean(timeseries[, i]) - sigma*measure_amp
    max <- mean(timeseries[, i]) + sigma*measure_amp
    
    min_inds <- which(timeseries[, i] < min)  
    ts[min_inds, i] <- min
    max_inds <- which(timeseries[, i] > max)
    ts[max_inds, i] <- max
    
    if (debug) {
      cat(sprintf("mean before clip: %2.4f, mean after clip %2.4f\n",
                  mean(timeseries[, i]), mean(ts[, i])))
    }
  }
...
data <- read.csv("~/Desktop/Astro/CIFAR Project/220429.FinalMatrices/1.csv", header=F)
size = 6
start <- rep(0, size)
steps = 100
start_index = size+1
...
covContemp <- matrix(1, size, size)
covLagged <- matrix(1, size, size)
ampContemp = 0.1
ampLagged = 0.1
ampMeasure = 1.0
...
coeff_draw_from_cov(ampLagged, matLagged, covLagged, maskLagged)

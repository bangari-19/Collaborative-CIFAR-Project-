...
ts.generate.asw <- function (mat, lvl, t,dens,p.group,con.b,lag.b,p.con) {
  repeat {
    ...
    repeat{
      ...
      indices.A <- indices.A[which(indices.A[,1]!=indices.A[,2]),]                                  ###kick out diagnonal (from A matrix only)
...
neg <- c(.3, -.3)                                     #to create the negative numbers
...
# #dir.create('/Users/reneehlozek/Code/CIFAR_Network/GIMME_TS_CHECK/data/')
# data.path <- '/Users/reneehlozek/Code/CIFAR_Network/GIMME_TS_CHECK/data/'
# # true.path <- 'C:/Users/tasha/Dropbox (Personal)/Research Projects/CIFAR/Sim_Study/balanced_in/true'
# # level.path <- 'C:/Users/tasha/Dropbox (Personal)/Research Projects/CIFAR/Sim_Study/balanced_in/levels'
...
#                             p.group = .50,dens = .20,
#                             con.b = sample(neg, 1), lag.b = sample(neg, 1))

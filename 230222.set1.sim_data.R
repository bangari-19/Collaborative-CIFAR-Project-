...
ts.generate.asw <- function (mat, lvl, t,dens,p.group,con.b,lag.b,p.con) {
  repeat {
    ...
    repeat{
      ...
      A[noise.inds]   <- A[noise.inds] + syncrng.box.muller(mu=0,sigma=0.01,nrow(noise.inds),seed=seed) 
      #rnorm(n = nrow(noise.inds), mean = 0, sd = .01)
      noise.inds      <- which(Phi != 0, arr.ind = TRUE)                                            ###add noise to Phi betas, SD =.1
      Phi[noise.inds] <- Phi[noise.inds] + syncrng.box.muller(mu=0,sigma=0.01,nrow(noise.inds),seed=seed)
      #rnorm(n = nrow(noise.inds), mean = 0, sd = .01)
      break
    }
...
dir.create('/Users/reneehlozek/Dropbox/CIFAR_Sims/230222_compare/')
data.path <- '/Users/reneehlozek/Dropbox/CIFAR_Sims/230222_compare/data'
true.path <- '/Users/reneehlozek/Dropbox/CIFAR_Sims/230222_compare/true'
level.path <- '/Users/reneehlozek/Dropbox/CIFAR_Sims/230222_compare/levels'
